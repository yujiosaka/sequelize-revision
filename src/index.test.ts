...
describe("SequelizeRevision", () => {
  ...
  describe("logging revisions", () => {
    ...
    it("logs revisions when updating a project", async () => {
      ...
      expect(revisions[2].id).toBe(project.revision);
      expect(revisions[2].model).toBe("Project");
      expect(revisions[2].document).toBe(
      ...
      expect(revisions[2].revision).toBe(revisions[2].id);
    });
    ...
    it("does not log revisions when failing a transaction", async () => {
      try {
        await sequelize.transaction(async (t) => {
          const table = await Project.create(
            { name: "sequelize-paper-trail", version: "1" },
            { transaction: t }
          );
          await table.update({ name: "sequelize-revision" });
  ...
  describe("logging revision changes", () => {
    ...
    it("logs revisionChanges when updating a project", async () => {
      ...
      expect(map(revisionChanges, "path")).toEqual([
        "name",
        "version",
        "name",
        "version",
